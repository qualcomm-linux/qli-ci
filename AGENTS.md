...
`qli-ci` owns a dedicated workflow to validate the full package loop against the
`qli-ci` ref under test:

- workflow: `.github/workflows/pkg-example-e2e-loop.yml`
- helper script: `scripts/pkg_example_e2e_loop.sh`
- sequence per lane: reset -> promote -> promotion PR build -> merge -> release
- test tags: `v1.0.0` and `v1.1.0`

### Job topology

- The e2e workflow is split into two jobs for GitHub UI clarity:
  - `debian-lane`
  - `ubuntu-lane`
- Jobs are sequential (not parallel): Ubuntu depends on Debian with
  `needs: [debian-lane]`.
- Lane state is handed off by artifact (`pkg-example-e2e-state-<run_id>`).

### Path toggles

The e2e loop supports repo-variable gating:

- `ENABLE_DEBIAN_PATH=0/1`
- `ENABLE_UBUNTU_PATH=0/1`

This allows temporarily disabling one lane while preserving the same workflow
contract and step layout. Gating is enforced at the job level so disabled lanes
show as skipped jobs in GitHub UI.

### Security/credentials

- `DEB_PKG_BOT_CI_TOKEN` is required for `pkg-example` clone/push, workflow
  dispatches, PR operations, and branch cleanup.
- Do not replace required-token behavior with silent fallback logic.

### Concurrency

- E2E runs use a global concurrency group and cancel in-progress runs on new
